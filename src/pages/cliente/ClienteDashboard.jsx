...
const ClienteDashboard = () => {
  ...
  const { agregarProducto } = useContext(CarritoContext); // ✅ Importante
  ...
  useEffect(() => {
    const fetchPedidos = async () => {
      ...
      const res = await fetch(`http://127.0.0.1:5000/api/sales-orders?user_id=${userId}`);
      const data = await res.json();
      setMisPedidos(data);

      // Obtener la última compra (último pedido)
  ...
  // Función para agregar producto al carrito con verificación de stock
  const handleAddToCart = async (producto) => {
    if (producto.stock <= 0) {
      alert("❌ Este producto no está disponible por stock bajo");
      return;
    }
    
    // Verificar si el producto ya está en el carrito
    const existingProduct = misPedidos.find(pedido => pedido.producto.id === producto.id);
    if (existingProduct) {
      alert("Este producto ya está en tu carrito");
      return;
    }

    // Reducir stock y agregar producto al carrito
    const updatedProducto = { ...producto, stock: producto.stock - 1 };
    agregarProducto(updatedProducto); // Add product to cart in context
    // Aquí puedes agregar lógica para actualizar el stock en el backend
  };
